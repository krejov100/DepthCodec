...
cv::Mat getOpenCVImage(const rs2::depth_frame& frame){
...
cv::Mat getOpenCVImage(const rs2::frame& frame){
    // Query frame size (width and height)
...
three::Image getOpen3DImage(const cv::Mat& frame){
    three::Image image;
...
three::Image getOpen3DImage(const rs2::depth_frame& frame){
    auto data = frame.get_data();
    three::Image image;
    image.PrepareImage(frame.get_width(), frame.get_height(), 1, 2);
    image.data_ = std::vector<uint8_t>(data, data + (frame.get_height() * frame.get_width() / sizeof(uint8_t)*2));
    return image;
}

three::Image getOpen3DImage(const rs2::frame& frame){
    auto data = frame.get_data();
    three::Image image;
    image.PrepareImage(frame.as<rs2::video_frame>().get_width(), frame.as<rs2::video_frame>().get_height(), 3, 1);
    image.data_ = std::vector<uint8_t>(data, data + (frame.as<rs2::video_frame>().get_height() * frame.as<rs2::video_frame>().get_width() / sizeof(uint8_t)));
    return image;
}
